...
## Notes
- Results are memoised on disk between sessions. Use `rxref_conf(cache = cachem::cache_mem())` for in-memory only.
- Be kind to the API; increase `rxref.rate_delay` if batching many requests.
- For strict reproducibility, we'll add a SQL backend bound to monthly RxNorm releases in a later version.
